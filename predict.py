...
from utils import get_bool_ids_greater_than, get_span, get_id_and_prob, cut_chinese_sent, dbc2sbc, logger


class UIEPredictor(object):
    def __init__(self, schema, device = 'cpu', position_prob = 0.5, max_seq_len = 512, batch_size = 64,
                 split_sentence = False, model_weight = ""):
        self._device = device
        ...
        if model_weight != "":
            if not os.path.exists(model_weight):
                ...
                logger.info("Start loading weight from " + model_weight)
    ...
    def _single_stage_predict(self, inputs):
        ...
        short_inputs = [{"text": short_input_texts[i], "prompt": short_texts_prompts[i]} for i in
                        range(len(short_input_texts))]
        
        token_ids, segment_ids, offset_maps = self._tokenizer.encode(short_texts_prompts, short_input_texts,
            ...
            if isinstance(s, str):
                ...
                for k, v in s.items():
                    if isinstance(v, str):
                        ...
                        raise TypeError(
                            "Invalid schema, value for each key:value pairs should be list or string but {} received".format(
                                type(v)))
...
if __name__ == '__main__':
    ...
    schema = ['部位与器官', '疾病与症状', '数量', '纹理', '密度', '边界', '边缘', '直径', '形态',
              '性质']  # Define the schema for entity extraction
    ie = UIEPredictor(schema = schema, model_weight = "./checkpoints/lung_uie_mcc_vat.pt")
    result = ie(sample)
    print(json.dumps(result, ensure_ascii = False))
    
    schema = {'疾病与症状': ['部位与器官', '数量', '密度', '边界', '边缘', '直径', '病灶形态', '性质', '异常所见'],
              '部位与器官': ['纹理', '部位形态', '部位密度']}
    ie = UIEPredictor(schema = schema, model_weight = "./checkpoints/lung_uie_mcc_vat.pt")
